...
*/
private["_vehicles","_control","_colorstring"];
...

{
	...
	//Color
	_colorstring = (_x select 8);
	
	if(_colorstring == "") then
	{
		_colorstring = "Standard";
	}
	else
	{
		_colorstring = round (parseNumber _colorstring);
		_colorstring = [_x select 2,_colorstring] call life_fnc_vehicleColorStr;
