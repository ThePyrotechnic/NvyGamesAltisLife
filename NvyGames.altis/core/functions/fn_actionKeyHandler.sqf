...
if(isNull _curTarget) exitWith 
{
	...
	{
		//Last action: mining if empty land
		//Try to MINE using action key [true] call life_fnc_pickaxeUse;
		//Check if there's a pickaxe
...
{
	if(playerSide == west) then //Wenn COP
	{
		...
		if(playerSide == civilian) then
		{
			if(_curTarget getVariable["restrained",false]) then
			{
				if(!dialog) then
				{
					...
					[_curTarget] call life_fnc_pInteraction_reb;
				}
				else
				{
					hint "There is already a dialog.";
...
life_action_inUse = false; //HACKFIX prevent action kill