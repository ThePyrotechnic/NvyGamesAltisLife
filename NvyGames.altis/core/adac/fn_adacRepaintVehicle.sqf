...
if(isNull _veh) exitWith { hint "No vehicle found!"; };
...
if(_index == -1) exitWith {hint "No color selected.";};
...
if((_veh isKindOf "Car") OR (_veh isKindOf "Ship") OR (_veh isKindOf "Air")) then
{
	if(([false,"uitem_adac_paint",_color_paint_buckets] call life_fnc_handleInv)) then
	{
		...
		if(player != vehicle player) exitWith {titleText["You have to get out of the vehicle in order to paint it can!","PLAIN"];};
		...
		/////////////////////
		titleText["Vehicle painted.","PLAIN"];
	}
	else
	{
