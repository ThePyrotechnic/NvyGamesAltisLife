...
if( ((gunner _veh) != player) && ((driver _veh) != player)) exitWith {hint "Can only be a gunner or driver!";}; 

/////////////////////////////////// PHASE 0: Ammo/Check for existance
//Get veh inventory
_inventory = _veh getVariable ["Trunk",[]];

if(count _inventory != 2) exitWith { hint "Vehicle has no inventory!"; };
_inventory = _inventory select 0;
_inventory = + _inventory; //copy the array and work in copy

//Only one can be used in time
if( _veh getVariable["bird_down_inuse", false] ) exitWith { hint "Weapon system is already working!"; };
...
if(!_hasweapon) exitWith { hint "You do not have the required weapon!"; _veh setVariable["bird_down_inuse", false];};
if(_ammocount < 1) exitWith { hint "You have no ammo!"; _veh setVariable["bird_down_inuse", false];};
if(!(alive _veh)) exitWith { hint "Vehicle is destroyed!"; _veh setVariable["bird_down_inuse", false];};
...
if(isNil("_target")) exitWith { hint "No target found!"; _veh setVariable["bird_down_inuse", false];};

hint parseText format["<img size='2' image='icons\schiessplatz.paa'/>Weapon system aimed at %1.", typeOf _target];
...
{	
	...
	hintSilent parseText format["<img size='2' image='icons\schiessplatz.paa'/>Ziel<br/>%1<br/><br/>Crew<br/>%3<br/>distance<br/><t size='1.2'>%2m / %4m</t><br/><br/>height difference<br/><t size='1.2'>%5m</t>", 
	typeOf _target, 
...
if(!_target_locked) exitWith { hint parseText "<img size='2' image='icons\schiessplatz.paa'/>Target operation aborted!"; _veh setVariable["bird_down_inuse", false];};
