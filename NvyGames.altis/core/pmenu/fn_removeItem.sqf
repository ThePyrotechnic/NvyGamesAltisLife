...
if(!([_value] call fnc_isnumber)) exitWith {hint "Invalid number"};
if(parseNumber(_value) <= 0) exitWith {hint "You must have to enter a number."};
...
if(player != vehicle player) exitWith {titleText["You cannot remove any items while you're in the vehicle.","PLAIN"]};
if(!([false,_data,(parseNumber _value)] call life_fnc_handleInv)) exitWith {hint "Specified amount could not be removed."};
