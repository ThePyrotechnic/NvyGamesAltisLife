...
if((lbCurSel 2005) == -1) exitWith {hint "You have no item selected that you want to give.";ctrlShow[2002,true];};
...
if(!([_val] call fnc_isnumber)) exitWith {hint "Invalid number.";ctrlShow[2002,true];};
if(parseNumber(_val) <= 0) exitWith {hint "Invalid number";ctrlShow[2002,true];};
if(isNil "_unit") exitWith {ctrlShow[2001,true]; hint "The selected player is not in range.";};
if(!([false,_item,(parseNumber _val)] call life_fnc_handleInv)) exitWith {hint "Can not give that much!";ctrlShow[2002,true];};
