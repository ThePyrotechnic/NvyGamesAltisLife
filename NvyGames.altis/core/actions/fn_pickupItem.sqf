...
if((time - life_action_delay) < 2) exitWith {hint "No spamming buttons!"};
...
if(playerSide == west && _illegal != -1) exitWith
{
	...
	titleText[format["%1 is an illegal Item! you get $%2 as a reward.",_itemName,[_impoundmoney] call life_fnc_numberText],"PLAIN"];
...
if(_diff <= 0) exitWith {hint "You have no more storage space."};
